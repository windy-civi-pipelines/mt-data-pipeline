...
2. **Name your new repository** using the convention: `Montana Data Pipeline` (e.g., `il-data-pipeline`, `tx-data-pipeline`).
...
3. **Update the state abbreviation** in both workflow files:
   ...
   **In `.github/workflows/scrape-and-format-data.yml`:**
   ...
   ```yaml
   env:
     STATE_CODE: mt # CHANGE THIS to your state abbreviation
   ...
   **In `.github/workflows/extract-text.yml`:**
   ...
   - name: Extract text
     ...
     with:
       state: mt # CHANGE THIS to your state abbreviation
   ```
...
uses: windy-civi/toolkit/actions/scrape@main
with:
  state: mt # State abbreviation (required)
...
git clone https://github.com/YOUR-ORG/Montana Data Pipeline
cd Montana Data Pipeline
